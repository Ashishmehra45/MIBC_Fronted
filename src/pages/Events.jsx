  ...
  return (
    <div className="min-h-screen bg-[#faf9f6] font-sans pb-24">
      ...
      <section className="pt-20 pb-16 text-center px-4">
        <h1 className="text-4xl md:text-5xl font-black text-[#A98842] mb-6 tracking-tight uppercase">
          {eventsContent.title}
        </h1>
      ...
      <section className="container mx-auto px-4 max-w-6xl">
        <div className="flex flex-col gap-10">
          {eventsContent.eventsList.map((event) => (
            <div
              key={event.id}
              className="bg-[#F2F6F9]/60 rounded-[32px] overflow-hidden flex flex-col md:flex-row shadow-sm hover:shadow-xl transition-all duration-500 border border-white group"
            >
              {/* Left Side: Event Image */}
              <div className="md:w-5/12 overflow-hidden h-full min-h-[320px]">
                <img
                  src={event.image}
                  alt={event.title}
              ...
              <div className="md:w-7/12 p-8 md:p-14 flex flex-col justify-center">
                <h2 className="text-2xl md:text-3xl font-black text-gray-900 mb-6 tracking-tight leading-[1.2]">
                  {event.title}
                </h2>
                ...
                {!event.isCalendar ? (
                  <>
                    <div className="space-y-1 mb-8">
                      <p className="text-[13px] font-black text-gray-950 tracking-tight uppercase">
                        Date: <span className="font-medium text-gray-600 ml-1">{event.date}</span>
                      </p>
                      <p className="text-[13px] font-black text-gray-950 tracking-tight uppercase">
                        Location: <span className="font-medium text-gray-600 ml-1">{event.location}</span>
                      </p>
                    </div>

                    <p className="text-gray-600 text-[15px] md:text-[16px] leading-loose font-medium mb-10 opacity-90">
                      {event.desc}
                    </p>
                  ...
                  /* Calendar Special Layout (Title ke baad Date, phir Blue List) */
                  <>
                    <p className="text-[13px] font-black text-gray-950 tracking-tight uppercase mb-6 opacity-60">
                      {event.date}
                    </p>
                    <p className="text-gray-600 text-[15px] md:text-[16px] leading-relaxed font-medium mb-8 opacity-90">
                      {event.desc}
                    </p>
                    <div className="space-y-4 mt-2">
                      {event.calendarItems.map((item, idx) => (
                        <div key={idx} className="flex items-center gap-3">
                          ...
                          <p className="text-gray-900 font-medium text-[15px] md:text-[17px]">
                            <span className="font-extrabold text-gray-900">{item.label}:</span> {item.value}
                          </p>
                        </div>
                      ))}
                    </div>

    ...
    <div className="min-h-screen bg-[#faf9f6] font-sans">
      ...
      <section className="pt-20 pb-12 text-center">
        <div className="container mx-auto px-4">
          <h1 className="text-4xl md:text-5xl font-black text-[#A98842] mb-4">
            Investment Facilitation
          </h1>
          <p className="max-w-3xl mx-auto text-gray-900 font-bold text-sm md:text-base leading-relaxed uppercase tracking-tighter">
        ...
        <div className="container mx-auto px-4 max-w-7xl">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-stretch">
            ...
            {investmentContent.sections.map((section, idx) => (
              <div
                key={idx}
                className="bg-white rounded-[40px] p-8 md:p-12 shadow-[0_20px_60px_rgba(0,0,0,0.03)] flex flex-col"
              >
                {/* Header of Card */}
                <div className="flex justify-between items-start mb-8">
                  ...
                  <span
                    className={`px-4 py-1.5 rounded-full text-[9px] font-black tracking-widest ${idx === 0 ? "bg-[#FFF9E6] text-[#A98842]" : "bg-[#1a1a1a] text-white"}`}
                  >
                    {section.badge}
                  </span>
                </div>
                ...
                {/* Fixed Alignment: Added min-h to description to keep checklists aligned */}
                <p className="text-gray-600 text-sm md:text-[15px] leading-relaxed mb-10 font-medium min-h-[80px]">
                  {section.desc}
                </p>

                {/* Checklist Points - Using flex-1 to push everything up or down as needed */}
                <div className="space-y-3 flex-1">
                  {section.points.map((point, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-4 bg-[#faf9f6]/50 p-4 rounded-2xl border border-gray-50 group hover:bg-white hover:shadow-md transition-all duration-300"
                    >
                      <div className="shrink-0 text-[#A98842]">
                        <Check size={16} strokeWidth={3} />
                      </div>
                      <span className="text-gray-700 text-[13px] md:text-sm font-bold tracking-tight">
                        {point}
                      </span>
                    </div>
                  ))}

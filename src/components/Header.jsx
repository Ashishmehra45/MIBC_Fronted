...
const Header = () => {
  ...
  // Nav link style generator - Standardized to Black with !important
  const getLinkStyle = (path) => {
    const isActive = location.pathname === path;
    return `text-[16px] font-bold tracking-tight transition-colors duration-300 ${
      isActive 
      ? "!text-[#b38e44]" // Active link gold
      : "!text-black hover:!text-[#b38e44]" // Forced black even in dark mode until hover
    }`;
  };
      ...
      <div className="container mx-auto px-4 lg:px-10">
        <div className="flex items-center justify-between h-20 md:h-24">
          ...
          <div className="flex-shrink-0">
            <Link to="/" className="flex items-center">
              <img
                className="h-10 md:h-12 w-auto transition-all text-black"
                src={logo}
                alt="MIBC Logo"
              />
            </Link>
          ...
          <nav className="hidden lg:flex items-center space-x-10">
            ...
            <div className="relative group">
              <button className={`${getLinkStyle("/services")} flex items-center gap-1 !text-black hover:!text-[#b38e44]`}>
                Services
              ...
              {/* Dropdown Menu - Links also forced to black */}
              <div className="absolute left-0 mt-2 w-64 bg-white shadow-2xl rounded-sm border border-gray-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 py-2">
                {[
                  ...
                  <Link
                    key={item.to}
                    to={item.to}
                    className="block px-6 py-3 text-[13px] font-bold !text-black hover:bg-gray-50 hover:!text-[#b38e44] transition-colors"
                  >
                    {item.label}
                  </Link>
                ))}
            ...
            <Link to="/initiatives" className={getLinkStyle("/initiatives")}>Initiatives</Link>
          ...
          <div className="flex items-center gap-6">
            <Link
              to="/membership"
              className="hidden md:block bg-[#b38e44] hover:bg-[#967635] text-white px-8 py-2.5 rounded-full text-[12px] font-bold tracking-wide transition-all shadow-md active:scale-95"
            >
              JOIN MIBC
            </Link>
            ...
            <button
              ...
              className="w-10 h-10 flex items-center justify-center rounded-full bg-[#b38e44]/10 text-[#b38e44] hover:bg-[#b38e44] hover:text-white transition-all duration-500"
            >
              {theme === "dark" ? (
        ...
        <div className={`lg:hidden transition-all duration-500 overflow-hidden ${isMobileMenuOpen ? "max-h-screen pb-10" : "max-h-0"}`}>
          <nav className="flex flex-col space-y-5 pt-6 border-t border-gray-50 bg-white">
            {["Home", "About", "Services", "Sectors", "Contact"].map((item) => (
              <Link
                key={item}
            ...
            <Link
              onClick={() => setIsMobileMenuOpen(false)}
              to="/membership"
              className="bg-[#b38e44] text-white text-center py-4 rounded-sm font-bold tracking-widest uppercase text-xs"
            >
              JOIN MIBC
            </Link>
          </nav>
